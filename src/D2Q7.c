...
int d2q7_getCell0(int r, int c, Field* pField);
int d2q7_getCell1(int r, int c, Field* pField);
int d2q7_getCell2(int r, int c, Field* pField);
int d2q7_getCell3(int r, int c, Field* pField);
int d2q7_getCell4(int r, int c, Field* pField);
int d2q7_getCell5(int r, int c, Field* pField);
int d2q7_getCell6(int r, int c, Field* pField);

MomentumList d2q7_listHeads[7];
...
void d2q7_initialiseCellState() {
    ...
    for( i = 0; i < 7; ++i ) {
        d2q7_listHeads[i].next = NULL;
        d2q7_listHeads[i].prev = NULL;
        d2q7_listHeads[i].count = 0;
        d2q7_listHeads[i].v.x = 0;
        d2q7_listHeads[i].v.y = 0;
    }
    ...
    for ( i = 0 ; i < 64; ++i ) {
        v = d2q7_computeMomentumFromHex( i );

        bitCount = countBits(i);

        pCurrent = &d2q7_listHeads[bitCount];
...
void d2q7_finaliseCellState() {
    ...
    // int j;
    // for ( j = 0; j < 6; ++j ) {
    //     pCurrent = d2q7_listHeads[j].next;
    //     while ( pCurrent != NULL ) {
    //         pCurrent = pCurrent->next;
    //         if ( pCurrent->prev ) {
    //             free( pCurrent->prev );
...
int d2q7_computeCollision(int inputHex, double random) {
    Vector v = d2q7_computeMomentumFromHex( inputHex );
    v.x = -v.x;
    v.y = -v.y;
    int bitCount = countBits(inputHex);
    MomentumList* pCurrent = &d2q7_listHeads[bitCount];
...
int d2q7_computeNewCell(int r, int c, Field* pField) {
    int cell0 = d2q7_getCell0(r, c, pField);
    int cell1 = d2q7_getCell1(r, c, pField);
    int cell2 = d2q7_getCell2(r, c, pField);
    int cell3 = d2q7_getCell3(r, c, pField);
    int cell4 = d2q7_getCell4(r, c, pField);
    int cell5 = d2q7_getCell5(r, c, pField);
    int cell6 = d2q7_getCell6(r, c, pField);
...
Vector d2q7_computeMomentum(int r, int c, Field* pField) {
    ...
    return d2q7_computeMomentumFromHex( value );
}
